$(document).ready(function () {
    $(".search-repair").click(function () {
        ...
        if (id !== "" && !isNaN(id)) {
            $.get("/cassa/getRiparazione", {"id": id}, function (data, status) {
                if (status == "success") {
                    if (!data) {
    ...
    $("#pagamento").click(function () {
        if ($("#statospan").text() === "Completata") {
            $.get("/cassa/getDatiPagamento", {"id": $("#repair-code").val()}, function (data, status) {
                if (status == "success") {
                    let prezzo = 0;
    ...
    $("#confirm-pay").click(function () {
        $.post("/cassa/pagamentoCompletato", {"id": $("#repair-code").val()}, function (data, status) {
            if (status === "success") {
